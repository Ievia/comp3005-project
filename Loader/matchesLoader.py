...
with psycopg.connect(f"dbname=comp3005finalproject user=postgres password=postgres") as conn:
    with conn.cursor() as cur:
        ...
        for f in matches_we_need:
            ...
            for row in matches:
                cur.execute("""
                    INSERT INTO match_competition (competition_id,
                                                   country_name,
                                                   competition_name)
                    VALUES (%s, %s, %s)
                ...
                """, (row['match_id'],
                      ...
                      row['away_score'],
                      row['match_status'],
                      row.get('match_status_360', None),
                      row.get('last_updated', None),
                      row.get('last_updated_360', None),
                      row.get('metadata', {}).get('data_version', None),
                      row.get('metadata', {}).get('shot_fidelity_version', None),
                      row.get('metadata', {}).get('xy_fidelity_version', None),
